...
class BendTwist(Energy):
    ...
    def d_energy_d_pos(pos: np.ndarray, theta: np.ndarray, solver_params: SolverParams):
        n = solver_params.n
        # First we compute partial E / partial theta_n
    ...
    def nabla_i_kb_i(pos: np.ndarray, i: int):
        ...
        den = np.linalg.norm(e_im1) * np.linalg.norm(e_i) + np.dot(e_im1, e_i)
