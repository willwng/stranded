...
class RodHelixConverter:
    ...
    def rod_to_helix(pos: cp.ndarray, theta: cp.ndarray, s: cp.ndarray = None) -> Helix:
        ...
        edge_frame_prev = cp.stack([t_prev_broadcasted, m_prev[0], m_prev[1]], axis=1)  # (101, 3, 3)
        edge_frame_next = cp.stack([t_next_broadcasted, m_next[0], m_next[1]], axis=1)  # (101, 3, 3)
